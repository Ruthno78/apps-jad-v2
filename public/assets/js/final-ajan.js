...
(function(){
  ...
  /* ---------------------------------------------------------------
   * V101 — CACHE «stale-while-revalidate» senp pou ekran yo pa rete
   * VID pandan yo ap tann rezo a. `swr(key, fetchFn, ttlMs)` retounen
   * IMEDYATMAN dènye kopi lokal la (si genyen) pandan li rafrechi an
   * background, epi li rele `onUpdate` si done fre yo diferan.
   * ------------------------------------------------------------- */
  window.Lotri.cache = (function(){
    const PREFIX = 'jl:swr:';
    function read(key){
      try { return JSON.parse(localStorage.getItem(PREFIX + key) || 'null'); } catch(_) { return null; }
    }
    function write(key, value){
      try { localStorage.setItem(PREFIX + key, JSON.stringify({ v: value, ts: Date.now() })); } catch(_) {}
    }
    /* Retounen { data, isStale } tousuit; rele fetchFn() an background
       epi `onFresh(data)` si li reyisi ak done chanje. */
    function swr(key, fetchFn, onFresh, ttlMs){
      ...
      if (cached && (!ttlMs || Date.now() - cached.ts < ttlMs)) {
        fresh(); // rafrechi an silans menm si li poko peryme
        return { data: cached.v, isStale: false };
      }
      fresh();
      return { data: cached ? cached.v : null, isStale: true };
    }
    return { swr, read, write };
  })();
})();
...
(function(){
  ...
  const LOCAL_FALLBACK = {
    ...
    wideDark: 'assets/img/jadstacklotto_logo-dark.png',
  ...
  window.Lotri.paintBrand = function(){
    ...
    if (!window.Lotri.v34) {
      document.querySelectorAll('[data-brand="wide"]').forEach(el => {
        const wideLocal = document.documentElement.dataset.theme === 'dark' ? LOCAL_FALLBACK.wideDark : LOCAL_FALLBACK.wide;
        mountBrandImg(el, b.logo_wide, b.name || 'JADSTACK LOTTO', wideLocal);
      });
...
(function(){
  ...
  /* V102 — Kòd kout jwèt yo sou fich la: 3 premye lèt non jwèt la + kantite
     chif pou fanmi LOTO yo (LOT3/LOT4/LOT5). BORLETTE ak MARIAGE pa gen
     sifiks chif paske yo pa varye. */
  const GAME_SHORT = { '20':'BOR', '30':'LOT3', '41':'LOT4', '42':'LOT4', '43':'LOT4',
                        '51':'LOT5', '52':'LOT5', '53':'LOT5', '60':'MAR' };
  window.Lotri.gameShortLabel = function(code, fallback){
    const c = String(code == null ? '' : code);
    if (GAME_SHORT[c]) return GAME_SHORT[c];
    const f = String(fallback || '').trim();
    return f ? f.toUpperCase().slice(0, 4) : c;
  };

  /* V102 — Fòma dat/lè konplè mande a: "le JJ/MM/AAAA" ak "HH:MM am/pm". */
  window.Lotri.fmtFicheDate = function(d){
    d = (d instanceof Date) ? d : new Date(d || Date.now());
    const p2 = n => String(n).padStart(2, '0');
    return 'le ' + p2(d.getDate()) + '/' + p2(d.getMonth() + 1) + '/' + d.getFullYear();
  };
  window.Lotri.fmtFicheTime = function(d){
    d = (d instanceof Date) ? d : new Date(d || Date.now());
...
<div class="fiche">
  ...
  <div class="meta">Réf. : ${esc(t.ref||'')}</div>
  <div class="meta">Tél. : ${esc(co.phone||'')}</div>
  <div class="meta"><span class="lbl-strong">Série :</span> ${esc(shortSerial)}</div>
  <div class="meta"><span class="lbl-strong">Date :</span> ${esc(t.date||'')}</div>
  <div class="meta">Heure : ${esc(t.time||'')}</div>
  <hr>
  <div class="row"><span><span class="lbl-strong">N° ticket :</span> ${esc(t.number||'')}</span></div>
  ${drawSections.map(sec => `
  <hr>
  <div class="row"><strong>${esc(sec.draw_name||'')}</strong></div>
  ${(sec.bets||[]).map(b=>{
    /* V103 — Mariage se toujou GRATIS: liy lan dwe montre
       "Mariage 51*27 => gratis" (nimewo a koupe an 2 ak yon '*' nan
       mitan) olye montan an, egzakteman jan modèl la mande. Tout lòt
       jwèt (Sèl/Borlette, Lotto3/4/5…) rete san chanjman, avèk montan
       an nòmalman. */
    const isMariage = String(b.game_code||'').toUpperCase().replace(/[^A-Z0-9]/g,'') === 'MARIAGE';
    if (isMariage) {
      const raw = String(b.number||'').replace(/\D/g,'');
      const mid = Math.ceil(raw.length/2);
      const pretty = raw.length >= 2 ? (raw.slice(0,mid) + ' * ' + raw.slice(mid)) : raw;
      return `<div class="row lot-row"><span>${esc(b.game_label||b.game_code||'')} ${esc(pretty)}</span><span>=&gt; gratis</span></div>`;
    }
    /* V104 — ti espasman ant boul yo lè plizyè nimewo kole ak '*'
       (egz. Lotto4 "12*12" → "12 * 12"): sèlman afichaj, valè b.number
       ki anrejistre nan BD la pa touche. */
    const displayNum = String(b.number||'').replace(/\s*\*\s*/g, ' * ');
    return `<div class="row lot-row"><span>${esc(b.game_label||b.game_code||'')} ${esc(displayNum)}</span><span>${Number(b.amount||0).toFixed(2)} ${esc(t.currency||'HTG')}</span></div>`;
  }).join('')}
  `).join('')}
  <hr>
  <div class="row"><strong>TOTAL</strong><strong>${Number(t.total||0).toFixed(2)}&nbsp;${esc(t.currency||'HTG')}</strong></div>
  ${t.mgAward ? `<div class="row mg-row">MARG&nbsp;${esc(t.mgAward.number||'')}&nbsp;&nbsp;${Number(t.mgAward.amount||0).toFixed(1)}${esc(t.currency||'HTG')}</div>` : ''}
  ...
  /* V99 — son kout jenere ak Web Audio (pa gen okenn fichye .mp3/.wav pou
   * pakete nan APK la — sekirite: pa janm kraze si Web Audio pa disponib). */
  /* V99-fix — "Mode silansye" : yon sèl flag nan localStorage ki kontwole
     TOUT son aplikasyon an rele. Defo se DEZAKTIVE (son yo ap jwe), jan sa
     te mande a. Panel Réglages la (pi ba nan fichye a) gen switch la. */
  window.JadStackSilent = window.JadStackSilent || {
    KEY: 'jadstack-silent-mode',
    isOn(){ try { return localStorage.getItem(this.KEY) === '1'; } catch(_) { return false; } },
    set(on){ try { localStorage.setItem(this.KEY, on ? '1' : '0'); } catch(_) {} }
  };
  window.JadStackSound = window.JadStackSound || (function(){
    let ctx = null;
    function ac(){ try { ctx = ctx || new (window.AudioContext||window.webkitAudioContext)(); return ctx; } catch(_) { return null; } }
    function tone(freq, start, dur, type, gain){
      if (window.JadStackSilent && window.JadStackSilent.isOn()) return;
      const c = ac(); if (!c) return;
      try {
        const o = c.createOscillator(), g = c.createGain();
        o.type = type || 'sine'; o.frequency.value = freq;
        g.gain.setValueAtTime(0.0001, c.currentTime + start);
        g.gain.exponentialRampToValueAtTime(gain || 0.15, c.currentTime + start + 0.02);
    ...
    return {
      printing(){ tone(1500, 0, 0.05, 'square', 0.05); },              // ti "tik" — enprime k ap kontinye
      success(){ tone(880, 0, 0.09, 'sine', 0.14); tone(1318, 0.09, 0.14, 'sine', 0.14); },
      fail(){ tone(300, 0, 0.16, 'sawtooth', 0.12); tone(190, 0.14, 0.22, 'sawtooth', 0.12); },
      click(){ tone(1100, 0, 0.035, 'sine', 0.06); }                    // ti son klik jeneral pou BTN
    };
  })();

  /* V99 — Enprime yon fich: rele pon natif la dirèkteman (AndroidPrinter),
   * JANM ouvri yon fenèt/popup navigatè sou Android (se sa k te bay "paj
   * bloke san BTN retou" a). window.open/print rete sèlman kòm sekou pou
   * tès sou navigatè òdinatè kote pa gen pon natif la ditou.
   * Retounen yon Promise<{ok, message}>. */
  window.Lotri.printFiche = async function(t){
    const bridgeOK = !!(window.AndroidPrinter);
    if (!bridgeOK) {
      // Sekou navigatè (deskop/tès) — pa konsène APK Android la.
      try {
        const w = window.open('', '_blank', 'width=380,height=640');
        if (w) {
          /* V99-fix — sèl 2 fichye CSS ki egziste reyèlman se tokens.css
             ak final-ajan.css (components.css pa t janm egziste — se poutèt
             sa fich la te soti san estil, ak "TOTALHTG" kole ansanm). Fenèt
             la fèmen otomatikman apre dyalòg enprime a fini (siksè oswa
             anile), pou li pa rete tounen yon tab "mò" san bouton retou. */
          w.document.write(`<html><head><title>Fiche</title>
            <link rel="stylesheet" href="assets/css/tokens.css"><link rel="stylesheet" href="assets/css/final-ajan.css">
            <style>body{display:flex;justify-content:center;padding:16px;background:#fff}</style>
            </head><body>${window.Lotri.renderFiche(t)}<script>
      ...
      return { ok: false, message: 'La fonction Q2i est disponible uniquement dans l\'APK Android.' };
    }
    if (!window.JadStackPrinter) return { ok: false, message: 'Module printer pa chaje.' };
    try {
      const r = await window.JadStackPrinter.printFicheV2(t);
      return r;
    } catch (e) {
      return { ok: false, message: 'Erreur impression : ' + ((e && e.message) || e) };
    }
  };
})();
 ...
 * ===================================================================== */
(function () {
  /* V99 fix: kèk chan modpas (egzanp: paj koneksyon #jl27-pass) deja gen
   * pwòp bouton "je" fèt alamen nan HTML la. Si otowrap la mache sou yo
   * tou, sa kreye 2 bouton "je" youn sou lòt. Detekte manyèlman bouton sa
   * a epi pa touche chan sa a ditou. */
  function hasManualEyeToggle(input) {
    const scope = input.closest('.jl27-pass') || input.parentElement;
    if (!scope) return false;
    const btn = scope.querySelector('button');
    if (!btn) return false;
    return /fa-eye/i.test(btn.innerHTML) ||
      /mot de passe|modpas|password/i.test(btn.getAttribute('aria-label') || '');
  }

  function wrap(input) {
    if (input.dataset.jlPwWrapped === '1') return;
    if (input.closest('.jl-pw-wrap')) { input.dataset.jlPwWrapped = '1'; return; }
    if (hasManualEyeToggle(input)) { input.dataset.jlPwWrapped = '1'; return; }
...
(function(){
  ...
  /* ---------- Limites de boules — V101 CACHE (san bloke antre done) ----------
     Ansyen vèsyon an te fè 2 apèl rezo (risk_limits + tout tikè tiraj la)
     SOU CHAK klik "Ajouter", kidonk ajan an te oblije tann entènèt pou
     chak boul. Kounye a nou gade cache a SENKRONIZMAN (pa gen await),
     epi nou rafrechi cache a an background. Garanti reyèl la (si limit
     la vrèman pase) rete SÈVÈ a (`jl9_rpc_create_ticket` deja tcheke l
     ankò anvan li kreye fich la) — kontwòl kliyan an se yon avètisman
     bonè, pa dènye ranpa a. */
  S._riskCache = { limits: null, ts: 0 };
  S._usedCache = {};

  S.warmBallLimits = async function(drawId){
    const SB = window.Lotri.supabase;
    try {
      if (!S._riskCache.limits || Date.now() - S._riskCache.ts > 5*60*1000) {
        const { data } = await SB.from('jl9_risk_limits')
          .select('max_amount,game_code,bet_number,scope').in('scope', ['number','agent_number']);
        S._riskCache.limits = data || []; S._riskCache.ts = Date.now();
      }
    } catch(_) {}
    if (!drawId) return;
    try {
      const c = S._usedCache[drawId];
      if (!c || Date.now() - c.ts > 20000) {
        ...
        const map = {};
        (rows||[]).forEach(r => (r.bets||[]).forEach(b => { map[b.n] = (map[b.n]||0) + Number(b.a||0); }));
        S._usedCache[drawId] = { data: map, ts: Date.now() };
      }
    } catch(_) {}
  };

  /* SENKRON — pa janm fè ajan an tann rezo pou antre yon pari. */
  S.checkBallLimit = function(drawId, gameCode, number, amount){
    try {
      const limits = S._riskCache.limits || [];
      const lim = limits.filter(l => String(l.bet_number) === String(number)
          && (!l.game_code || l.game_code.toLowerCase() === String(gameCode).toLowerCase()))
        .sort((a,b)=> a.max_amount - b.max_amount)[0];
      if (!lim || !lim.max_amount) return null;
      const used = (S._usedCache[drawId] && S._usedCache[drawId].data[number]) || 0;
      if (used + Number(amount) > Number(lim.max_amount)) {
        return 'Limite de la boule ' + number + ' presque atteinte (' + used + ' / ' + lim.max_amount + '). Le serveur pourra refuser ce pari.';
      }
      return null;
    } catch(_) { return null; }
  };
...
window.LotriShell = (function(){
  ...
  async function mount(menu, brandLabel){
      ...
      <div class="shell" id="shell">
        ...
        <aside class="sidebar">
          ...
          <div class="side-scroll">
            ...
            ${menuHtml(menu)}
            <div class="side-signature" aria-hidden="true" style="margin-top:auto;padding:1rem .6rem 1.1rem;text-align:center;font-size:.62rem;line-height:1.5;letter-spacing:.03em;color:var(--muted,#8a8f98);opacity:.55;user-select:none;pointer-events:none">DEVLOPER<br>RHTN7877 &amp; JADBONY<br>DROIT ©<br>TOUTES DROIT son réserve a jadstaklotto</div>
          </div>
        </aside>
    ...
    /* V103 — Bwat shell la ap boote la a. Konfigirasyon (jl9_site_config,
       ki gen ladan konfig tikèt la) dwe rive an DIRÈK: si Super-Admin
       chanje l pandan yon Agent gentan konekte, Agent la pa dwe bezwen
       rechaje paj la — loadConfig() + paintBrand() re-woule otomatikman. */
    try {
      if (!window.Lotri._siteConfigRt) {
        const scName = 'jl103-siteconfig-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8);
        let scDebounce = null;
        window.Lotri._siteConfigRt = window.Lotri.supabase.channel(scName)
          .on('postgres_changes', { event: '*', schema: 'public', table: 'jl9_site_config' }, () => {
            clearTimeout(scDebounce);
...
(function () {
  ...
  /* ------------------- FAB (rezilta tiraj) — RETIRE nèt (demand explicit).
     BTN flotan dis la te fè doub anplwa ak lyen "Résultats des tirages" ki
     deja nan menu/bottom-nav lan; li te ret kole sou tout paj yo (menm sou
     paj vant lan) san rezon. buildFab() rete kòm no-op pou pa kase okenn
     lòt apèl ki ta rele li. */
  function buildFab() { document.querySelector('.jl-fab')?.remove(); }
...
(function () {
  ...
  const Printer = {
    ...
    async print(text) {
      ...
      if (!this.isLive()) {
        /* V101 — flux demandé: Press PRINT -> Show Paired Bluetooth
         * Printers -> Select Printer -> Connect -> Print. connectSmart()
         * réutilise le dernier choix, sinon propose la liste des
         * imprimantes déjà appairées (n'importe quelle marque/modèle). */
        const c = await this.connectSmart();
        if (!c.ok) return c;
      }
    ...
    /* ---- V101 — Chwazi printer nan lis "Paired Bluetooth Printers" ---- */

    /* Kle nan localStorage kote nou sonje dènye printer itilizatè a chwazi,
     * pou pa mande l chwazi chak fwa li vle enprime. */
    _SAVED_KEY: 'jadstack_printer_address',

    getSavedPrinter() {
      try { return localStorage.getItem(this._SAVED_KEY) || ''; } catch (_) { return ''; }
    },
    setSavedPrinter(address, label) {
      try {
        localStorage.setItem(this._SAVED_KEY, address || '');
        localStorage.setItem(this._SAVED_KEY + '_label', label || '');
      } catch (_) { /* ignore */ }
    },

    /* Retounen [{name, address}, ...] — tout aparèy Bluetooth ki deja
     * "paired" sou telefòn nan, kèlkeswa mak/modèl (Q2i, GOOJPRT, lòt
     * POS/tèminal Bluetooth, elatriye). */
    async listPrinters() {
      const b = bridge();
      if (!b || typeof b.listPairedDevices !== 'function') return [];
      const perm = await this.ensurePermissions();
      if (perm !== 'OK' && perm !== 'GRANTED' && perm !== true) return [];
      try {
        const raw = await callBridge('listPairedDevices');
        const list = JSON.parse(raw || '[]');
        return Array.isArray(list) ? list : [];
      } catch (_) { return []; }
    },

    /* Konekte sou yon printer espesifik (adrès MAC), sonje l pou pwochen
     * fwa, epi mete drapo `connected` a ajou. */
    async connectTo(address, label) {
      if (!bridge()) return { ok: false, message: NO_BRIDGE };
      const perm = await this.ensurePermissions();
      if (perm !== 'OK' && perm !== 'GRANTED' && perm !== true) {
        this.connected = false;
        return { ok: false, message: String(perm || 'Permission Bluetooth refusée.') };
      }
      const r = await callBridge('connect', address);
      if (r === 'OK' || r === 'CONNECTED') {
        this.connected = true;
        this.setSavedPrinter(address, label);
        return { ok: true, message: (label || 'Printer') + ' connecté ✅' };
      }
      this.connected = false;
      return { ok: false, message: String(r || 'Connexion échouée.') };
    }
  };

  /* Montre yon ti modal senp pou itilizatè a chwazi youn nan printer
   * Bluetooth ki deja paired yo. Retounen Promise<{address,name}|null>. */
  Printer.pickPrinter = async function () {
    const list = await Printer.listPrinters();
    if (!list.length) return null;
    if (list.length === 1) return list[0];
    return new Promise((resolve) => {
      const old = document.getElementById('jl-printer-picker');
      if (old) old.remove();
      const overlay = document.createElement('div');
      overlay.id = 'jl-printer-picker';
      ...
      overlay.innerHTML = `
        <div style="background:#fff;width:100%;max-width:420px;border-radius:14px 14px 0 0;padding:16px;max-height:70vh;overflow:auto">
          <h3 style="margin:0 0 10px;font-size:1rem">Choisir une imprimante Bluetooth</h3>
          <div id="jl-pp-list"></div>
          <button type="button" id="jl-pp-cancel" class="btn btn-ghost" style="width:100%;margin-top:10px">Annuler</button>
        </div>`;
      document.body.appendChild(overlay);
      const box = overlay.querySelector('#jl-pp-list');
      list.forEach((d) => {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'btn btn-ghost';
        btn.style.cssText = 'display:flex;align-items:center;gap:.5rem;width:100%;text-align:left;margin-bottom:6px';
        btn.innerHTML = '<i class="fa-solid fa-print"></i> ' + (d.name || 'Aparèy san non') + '<span style="margin-left:auto;font-size:.7rem;color:#888">' + (d.address || '') + '</span>';
        btn.onclick = () => { overlay.remove(); resolve(d); };
        box.appendChild(btn);
      });
      overlay.querySelector('#jl-pp-cancel').onclick = () => { overlay.remove(); resolve(null); };
    });
  };

  /* Sikilè "san bloke" pou konekte anvan enprime: si gen yon printer
   * deja sonje (localStorage), eseye l dirèkteman; sinon, si gen plis
   * pase 1 aparèy paired, montre pickPrinter(). JAMEN kraze si okenn
   * printer pa jwenn — retounen yon mesaj klè. */
  Printer.connectSmart = async function () {
    if (!bridge()) return { ok: false, message: NO_BRIDGE };
    const perm = await Printer.ensurePermissions();
    if (perm !== 'OK' && perm !== 'GRANTED' && perm !== true) {
      return { ok: false, message: String(perm || 'Permission Bluetooth refusée.') };
    }
    const saved = Printer.getSavedPrinter();
    if (saved) {
      const r = await Printer.connectTo(saved);
      if (r.ok) return r;
      // Printer sonje a pa reponn (chanje/dekonekte) — repase pa lis la.
    }
    const list = await Printer.listPrinters();
    if (!list.length) {
      return { ok: false, message: 'Aucune imprimante Bluetooth appairée. Allez dans les paramètres Bluetooth du téléphone pour appairer votre imprimante.' };
    }
    const chosen = list.length === 1 ? list[0] : await Printer.pickPrinter();
    if (!chosen) return { ok: false, message: 'Impression annulée — aucune imprimante choisie.' };
    return Printer.connectTo(chosen.address, chosen.name);
  };
  ...
  /* ======================================================================
   * V99 — Motè ESC/POS #2 : fòma REYÈL fich la (menm estrikti/kontni ak
   * window.Lotri.renderFiche() ki afiche nan Aperçu a), ak lajè ki adapte
   * otomatikman ak papye a (58mm≈32 car / 80mm≈48 car).
   *
   * IMPORTANT — pa touche buildReceipt() pi wo a: se li Printer.testPrint()
   * itilize toujou, san okenn chanjman, jan sa te mande a.
   *
   * `t` gen menm fòm ak objè renderFiche() aksepte a:
   *   { company:{name,address,phone,logo_url}, ref, serial, number,
   *     date, time, currency, total,
   *     draws:[{draw_name, bets:[{game_code,game_label,number,amount}]}] }
   * ==================================================================== */
  function paperWidthChars(mm) {
    // 58mm ≈ 32 caractè, 80mm ≈ 48 caractè sou pifò printer thermal ESC/POS.
    return Number(mm) >= 70 ? 48 : 32;
  }

  function wrapCentered(text, width) {
    text = clean(text);
    if (!text) return '';
    ...
    words.forEach(w => {
      ...
      if (test.length > width) { if (cur) lines.push(cur); cur = w; }
      else cur = test;
    });
  ...
  function twoCol(left, right, width) {
    ...
    if (left.length + right.length + 1 > width) {
      // liy twò long: koupe pati goch la pou pati dwat (montan) toujou li klè
  ...
  function buildReceiptV2(t, opts) {
    t = t || {};
    opts = opts || {};
    const cfg = (window.Lotri && window.Lotri.config && window.Lotri.config.ticket) ||
                (window.JADSTACK_DEFAULTS && window.JADSTACK_DEFAULTS.ticket) || {};
    const width = paperWidthChars(opts.paperWidthMm ||
      (window.Lotri && window.Lotri.config && window.Lotri.config.ticket && window.Lotri.config.ticket.paper_width_mm) || 58);

    const co = t.company || {};
    const draws = Array.isArray(t.draws) && t.draws.length
      ? t.draws
      : [{ draw_name: t.draw_name || '', bets: t.bets || [] }];
    const currency = clean(t.currency || 'HTG');

    let out = '';
    out += '\x1B\x40';                 // init
    out += '\x1B\x61\x01';             // center
    out += '\x1B\x45\x01';             // bold ON
    out += (clean(co.name || 'JADSTACK LOTTO').toUpperCase()) + '\n';
    out += '\x1B\x45\x00';             // bold OFF
    if (co.address) out += clean(co.address) + '\n';
    out += 'Réf. : ' + clean(t.ref || '') + '\n';
    out += 'Tél. : ' + clean(co.phone || '') + '\n';
    const shortSerial = clean(t.serial || '').split('-')[0].toUpperCase();
    // V104 — Etikèt Série/Date diminye: gra NÒMAL (san doub gwosè V103 la); valè a rete nòmal tou. N° ticket ap rete jan li ye.
    if (shortSerial) out += '\x1B\x45\x01' + 'Série :' + '\x1B\x45\x00' + ' ' + shortSerial + '\n';
    out += '\x1B\x45\x01' + 'Date :' + '\x1B\x45\x00' + ' ' + clean(t.date || '') + '\n';
    out += 'Heure : ' + clean(t.time || '') + '\n';
    out += '\x1B\x61\x00';             // gòch
    out += line(width) + '\n';
    out += '\x1D\x21\x01\x1B\x45\x01' + 'N° ticket :' + '\x1B\x45\x00\x1D\x21\x00' + ' ' + clean(t.number || '') + '\n';

    // V103 — Plis espas ant chak liy nimewo boul: ogmante espas liy lan
    // (defo yon printer ESC/POS se ~30 pwen) pandan seksyon pari yo,
    // epi remèt espas nòmal la apre ("\x1B\x32").
    draws.forEach(sec => {
      out += line(width) + '\n';
      out += '\x1B\x45\x01' + clean(sec.draw_name || '') + '\x1B\x45\x00' + '\n';
      out += '\x1B\x33' + String.fromCharCode(42); // espas ant liy pi laj
      (sec.bets || []).forEach(b => {
        const g = clean(b.game_label || b.game_code || '');
        const isMariage = String(b.game_code||'').toUpperCase().replace(/[^A-Z0-9]/g,'') === 'MARIAGE';
        if (isMariage) {
          const raw = String(b.number||'').replace(/\D/g,'');
          const mid = Math.ceil(raw.length/2);
          const num = raw.length >= 2 ? (raw.slice(0,mid) + ' * ' + raw.slice(mid)) : clean(b.number||'');
          out += twoCol((g ? g + ' ' : '') + num, '=> gratis', width) + '\n';
          return;
        }
        // V104 — menm ti espasman an sou papye a pou lòt jwèt ki gen '*'
        // (egz. Lotto4 "12*12" → "12 * 12"); zafè afichaj sèlman, pa
        // touche valè k ap sèvi pou kalkil/verifikasyon yo.
        const num = clean(b.number || '').replace(/\s*\*\s*/g, ' * ');
        const left = (g ? g + ' ' : '') + num;
        const right = Number(b.amount || 0).toFixed(2) + ' ' + currency;
        out += twoCol(left, right, width) + '\n';
      });
      out += '\x1B\x32'; // remèt espas liy nòmal
    });

    out += line(width) + '\n';
    out += '\x1B\x45\x01';
    out += twoCol('TOTAL', currency + ' ' + Number(t.total || 0).toFixed(2), width) + '\n';
    out += '\x1B\x45\x00';
    // V102 — Mariage GRATUIT (MARG): SÈL liy sa a, an ba, santre, an gra
    // e yon ti kras pi gro pase rès lo yo, pou l dekole san l pa domine.
    if (t.mgAward) {
      out += line(width) + '\n';
      out += '\x1B\x61\x01' + '\x1D\x21\x01\x1B\x45\x01';
      out += 'MARG ' + clean(t.mgAward.number || '') + '  ' + Number(t.mgAward.amount || 0).toFixed(1) + currency + '\n';
      out += '\x1B\x45\x00\x1D\x21\x00' + '\x1B\x61\x00';
    }
    out += line(width) + '\n';
    out += '\x1B\x61\x01';
    if (cfg.dotted) out += clean(cfg.dotted) + '\n';
    if (cfg.system_name) out += '\x1B\x45\x01' + clean(cfg.system_name) + '\x1B\x45\x00' + '\n';
    if (cfg.legal) out += wrapCentered(cfg.legal, width);
    out += '\x1B\x61\x00';
    out += '\n\n\n\n';
    return out;
  }

  window.buildJadStackReceiptV2 = buildReceiptV2;

  /* Enprime yon fich (fòma reyèl v99) dirèkteman sou pon natif la.
   * Retounen { ok, message }. Pa janm ouvri okenn popup/fenèt navigatè. */
  Printer.printFicheV2 = async function (t, opts) {
    if (!bridge()) return { ok: false, message: NO_BRIDGE };
    if (!this.isLive()) {
      const c = await this.connect();
      if (!c.ok) return c;
    }
    const receipt = buildReceiptV2(t, opts);
    const r = await callBridge('print', receipt);
    if (r === 'PRINT_OK' || r === 'OK') return { ok: true, message: 'Imprimer ✅' };
    if (String(r).indexOf('non connecté') >= 0 || String(r).indexOf('pa konekte') >= 0) this.connected = false;
    return { ok: false, message: String(r || "Échec de l'impression.") };
  };
...
(function () {
  ...
  function ficheToPrintData(fiche, ticket) {
    ...
    return {
      ...
      extra: {
        ...
        seller: seller,
        companyName: ((window.__lotriCompany && window.__lotriCompany.name) || (JSON.parse(localStorage.getItem('lotriCompany') || '{}').name) || ''),
  ...
  /* V103 — Paj Bluetooth refèt: pa yon kopi, se PWÒP style nou, ak plis
     fonksyon reyèl (tout branche sou window.JadStackPrinter ki deja
     egziste — okenn nouvo API bridge envante):
       · Kat "Aparèy konekte kounye a" (non + adrès sonje).
       · Lis "Aparèy Bluetooth disponib" (deja pè sou telefòn nan),
         tape sou youn pou konekte, ak bouton "Actualiser".
       · Fòm minimal pou konekte ak yon adrès MAC antre alamen (ekivalan
         "konfigire ak seri pèsonalize" san envante yon API ki pa la).
       · Test / Imprimer dènye tikè rete jan yo te ye. */
  function renderPrinterView(host) {
    if (!host) return;
    const P = Q2I();
    const s = statusText();
    const saved = P && P.getSavedPrinter ? P.getSavedPrinter() : '';
    const savedLabel = (() => {
      try { return P ? (localStorage.getItem(P._SAVED_KEY + '_label') || '') : ''; } catch (_) { return ''; }
    })();
    host.innerHTML = `
      <div class="printer-option-page jl103-bt-page">
        <div class="card">
          <div class="card-hd">
            <h3><i class="fa-solid fa-print"></i> Imprimante Bluetooth</h3>
            <span class="q2i-status ${s.cls}">${s.label}</span>
          </div>
          <p class="muted printer-option-note">
            Gérez votre imprimante Bluetooth ici : appareil connecté, appareils disponibles, test d'impression.
          </p>

          <div class="jl103-bt-connected">
            <i class="fa-solid ${saved ? 'fa-print' : 'fa-circle-question'}"></i>
            <div>
              <div class="jl103-bt-connected-label">Appareil associé</div>
              <div class="jl103-bt-connected-name">${esc(savedLabel || saved || 'Aucun appareil enregistré')}</div>
              ${saved ? `<div class="jl103-bt-connected-addr">${esc(saved)}</div>` : ''}
            </div>
          </div>

          <div class="q2i-actions printer-option-actions">
            <button type="button" class="btn btn-secondary" data-q2i-connect>🖨️ Connecter</button>
            <button type="button" class="btn btn-ghost" data-q2i-test>Tester l'imprimante</button>
            <button type="button" class="btn btn-primary" data-q2i-print ${lastTicket ? '' : 'disabled'}>🖨️ Imprimer le dernier ticket</button>
          </div>
        </div>

        <div class="card">
          <div class="card-hd">
            <h3><i class="fa-solid fa-magnifying-glass"></i> Appareils disponibles</h3>
            <button type="button" class="btn btn-ghost btn-sm" data-bt-refresh><i class="fa-solid fa-rotate"></i> Actualiser</button>
          </div>
          <div id="jl103-bt-list" class="jl103-bt-list"><div class="spinner"></div></div>
        </div>

        <div class="card">
          <div class="card-hd"><h3><i class="fa-solid fa-keyboard"></i> Connexion manuelle</h3></div>
          <p class="muted printer-option-note">Si votre appareil n'apparaît pas dans la liste, entrez son adresse Bluetooth (MAC).</p>
          <form id="jl103-bt-manual" class="jl103-bt-manual">
            <input class="input" type="text" id="jl103-bt-addr" placeholder="00:AA:11:BB:22:CC" autocomplete="off">
            <input class="input" type="text" id="jl103-bt-name" placeholder="Nom (optionnel)" autocomplete="off">
    ...
    host.querySelector('[data-bt-refresh]')?.addEventListener('click', () => loadBtList(host));
    host.querySelector('#jl103-bt-manual')?.addEventListener('submit', async (e) => {
      e.preventDefault();
      const addr = host.querySelector('#jl103-bt-addr').value.trim();
      const name = host.querySelector('#jl103-bt-name').value.trim();
      if (!addr) { notify('Saisissez une adresse Bluetooth.', 'error'); return; }
      const P = Q2I();
      if (!P) { notify('Le module Q2i n\'est pas chargé.', 'error'); return; }
      const r = await P.connectTo(addr, name || addr);
  ...
  async function loadBtList(host) {
    const box = host.querySelector('#jl103-bt-list');
    if (!box) return;
    const P = Q2I();
    if (!P || !P.available()) {
      box.innerHTML = '<p class="muted" style="padding:.4rem">Disponible uniquement dans l\'application Android.</p>';
      return;
    }
    const list = await P.listPrinters();
    if (!list.length) {
      box.innerHTML = '<p class="muted" style="padding:.4rem">Aucun appareil Bluetooth appairé. Associez d\'abord l\'imprimante dans les paramètres Bluetooth du téléphone.</p>';
      return;
    }
    const saved = P.getSavedPrinter ? P.getSavedPrinter() : '';
    box.innerHTML = list.map(d => `
      <button type="button" class="jl103-bt-item${d.address === saved ? ' is-connected' : ''}" data-bt-addr="${esc(d.address || '')}" data-bt-name="${esc(d.name || '')}">
        <i class="fa-solid fa-print"></i>
        <span class="jl103-bt-item-name">${esc(d.name || 'Appareil sans nom')}</span>
        <span class="jl103-bt-item-addr">${esc(d.address || '')}</span>
        ${d.address === saved ? '<i class="fa-solid fa-circle-check jl103-bt-item-check"></i>' : ''}
      </button>`).join('');
    box.querySelectorAll('[data-bt-addr]').forEach(btn => {
      btn.addEventListener('click', async () => {
        const r = await P.connectTo(btn.dataset.btAddr, btn.dataset.btName);
        notify(r.message, r.ok ? 'success' : 'error');
        paint();
        loadBtList(host);
      });
...
(function(){
  ...
  LotriShell.register('pos', {
    render: async (host)=>{
      const prof = window.__lotriProfile || {};
      /* V103 — perf "Fè yon fiche": ansyen kòd la te fè 3 apèl rezo SOU
         SWIT (loadGameRules() → jl9_rpc_tick_draws → Promise.all), kidonk
         ajan an te tann 3 wonn rezo anvan li wè anyen sou ekran an.
         (1) jl9_rpc_tick_draws retire nèt: pg_cron (jl12_close_due)
             deja fèmen/ouvri tiraj yo chak minit sou sèvè a — apèl la
             te fè menm travay la 2 fwa pou anyen.
         (2) loadGameRules() kounye a woule AN PARALÈL ak lòt 3 rekèt
             yo (draws/games/company) olye li tann yo. */
      const [, {data:draws},{data:games},{data:company}] = await Promise.all([
        loadGameRules(),                            /* V13 §A11 */
      ...
      /* V99 — "3 dènye jwèt jwe" yo parèt an pi PITI (chip konpak) nan lis
         jwèt la, pou moun ka rekonèt rapid youn nan 3 dènye jwèt li te
         itilize san yo pa pran menm plas ak lòt jwèt yo. Sovgade pa ajan
         (localStorage), max 3, pi resan an dabò. */
      const RECENT_KEY = 'jl_recent_games_' + (prof.id || 'anon');
      function getRecentGames(){
        try { return JSON.parse(localStorage.getItem(RECENT_KEY) || '[]'); }
        catch(_) { return []; }
      }
      function isRecentGame(code){ return getRecentGames().includes(code); }
      function pushRecentGame(code){
        if (!code) return;
        let list = getRecentGames().filter(c => c !== code);
        list.unshift(code);
        list = list.slice(0, 3);
        ...
        <article class="vente-panel-v88 vente-saisie-v88">
          ...
          <div class="vente-champ-v88">
            ...
            <div class="vente-jeux-v88" id="vente-jeux" aria-label="Liste des jeux">
              ${(games||[]).map((g,i)=>`<button type="button" class="vente-jeu-v88${isRecentGame(g.code)?' vente-jeu-v88--recent':''}" data-code="${esc(g.code)}" aria-pressed="${i===0?'true':'false'}">${esc(g.name||g.code)}</button>`).join('')}
            </div>
          </div>
      ...
      /* V97 — Lis jwèt yo break sou plizyè liy (max 3). Si kontni an ta
         mande PLIS pase 3 liy, n ap bascul sou yon sèl liy ak scroll
         orizontal olye l kontinye desann san limit. */
      function layoutGamesRail(){
        if (!tabs) return;
        tabs.classList.remove('vente-jeux-v88--scroll');
        // Hauteur reyèl kontni an ta pran si l gen dwa break san limit.
        const natural = tabs.scrollHeight;
        const maxAllowed = tabs.clientHeight; // limite pa max-height (3 liy) nan CSS
        if (natural > maxAllowed + 1) {
          tabs.classList.add('vente-jeux-v88--scroll');
        }
      }
      layoutGamesRail();
      window.addEventListener('resize', layoutGamesRail, { passive: true });
      ...
      function setGame(code){
        ...
        /* V99 — mete jaje sa a nan 3 dènye jwèt yo, epi rafrechi ki chip
           ki dwe parèt an pi piti (konpak) nan lis la. */
        pushRecentGame(code);
        const recent = getRecentGames();
        tabs.querySelectorAll('.vente-jeu-v88').forEach(b=>
          b.classList.toggle('vente-jeu-v88--recent', recent.includes(b.dataset.code)));
        layoutGamesRail();
      }
      ...
      sel.addEventListener('change', ()=>{
        ...
        window.Lotri.security.warmBallLimits(sel.value); // V101 — background, pa bloke UI a
      });
      setGame(game);
      ...
      /* V104 — "Rejouer" (soti nan Détails du ticket, paj Fiche): si gen
         yon fich k ap tann pou rejwe, nou eseye pre-ranpli fich la SÈLMAN
         si menm tiraj la (menm non) toujou louvri kounye a. Nou pa kreye
         ni enprime anyen otomatikman — ajan an toujou dwe tcheke epi peze
         "Imprimer" li menm, kidonk validasyon sèvè a toujou aplike. */
      if (window.__lotriReplayQueue) {
        const rq = window.__lotriReplayQueue;
        window.__lotriReplayQueue = null;
        const match = (draws || []).find(dr =>
          String(dr.name || '').trim().toLowerCase() === String(rq.drawName || '').trim().toLowerCase());
        if (match && Array.isArray(rq.bets) && rq.bets.length) {
          let l = findLine(match.id);
          if (!l) { l = { draw_id: match.id, draw_name: match.name, bets: [] }; lines.push(l); }
          rq.bets.forEach(b => l.bets.push({ n: b.n, a: b.a, game_code: b.game_code }));
          refresh();
          window.Lotri.popupAlert(`Fiche rechargée pour ${esc(match.name)} — vérifiez avant d'imprimer.`, 'success');
        } else {
          window.Lotri.popupAlert(
            `Le tirage « ${esc(rq.drawName || '')} » n'est pas ouvert en ce moment — impossible de recharger automatiquement la fiche. Ajoutez les paris manuellement.`,
            'warning');
        }
      }

      /* V101 — Lis tiraj yo dwe rete SENKRONIZE ak sa k ap fèmen an tan
         reyèl: yo pa dwe janm rete afiche kòm "chwazisab" apre yo fèmen.
         Olye nou kite ajan an eseye vann epi resevwa erè sèvè a
         ("Tiraj X fèmen"), nou retire tiraj la otomatikman nan lis la
         (ak nan fich an kou si li te deja ladan), pandan k backend la
         (jl12_close_due, chak minit) fèmen l e trigger Maryaj Kado a
         kontinye fonksyone san okenn chanjman. */
      let drawsCache = draws || [];
      async function refreshDrawList(){
        const { data: fresh, error } = await SB()
          .from('jl24_draws_today')
          .select('id,name,status,game_code,closes_at,sales_open,sort_order')
          .eq('sales_open', true).order('sort_order').order('name');
        if (error || !Array.isArray(fresh)) return;
        const openIds = new Set(fresh.map(d => d.id));
        const removedNow = drawsCache.filter(d => !openIds.has(d.id));
        drawsCache = fresh;

        const prevValue = sel.value;
        sel.innerHTML = fresh.map(d=>`<option value="${d.id}" data-gc="${esc(d.game_code||'')}">${esc(d.name)}</option>`).join('')
          || '<option value="">Aucun tirage ouvert</option>';
        if (openIds.has(prevValue)) sel.value = prevValue;
        else if (fresh[0]) { sel.value = fresh[0].id; sel.dispatchEvent(new Event('change')); }

        // Retire dirèkteman nan fich an kou tout liy ki poko soumèt sou yon
        // tiraj ki fèk fèmen — pou ajan an pa janm bite sou erè "fèmen".
        if (removedNow.length){
          let touched = false;
          removedNow.forEach(d => {
            const idx = lines.findIndex(l => l.draw_id === d.id);
            if (idx !== -1) { lines.splice(idx, 1); touched = true; }
          });
          if (touched) {
            refresh();
            window.Lotri.popupAlert(
              (removedNow.length === 1 ? `Le tirage ${removedNow[0].name} vient de fermer — retiré de la fiche.` :
               'Un ou plusieurs tirages viennent de fermer — retirés de la fiche.'),
              'warning'
            );
          }
        }
      }
      const drawPoll = setInterval(refreshDrawList, 20000);
      /* V103 — Realtime sou jl9_draws: lè yon tiraj ouvè/fèmen, lis la
         chanje SANLATAN olye li tann jiska 20s (poll la rete kòm filè
         sekirite si Realtime ta dekonekte tanporèman). */
      let drawRtChannel = null;
      try {
        const drawRtName = 'jl103-draws-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8);
        drawRtChannel = SB().channel(drawRtName)
          .on('postgres_changes', { event: '*', schema: 'public', table: 'jl9_draws' }, () => refreshDrawList())
          .subscribe();
      } catch (_) {}
      const stopDrawPoll = () => {
        clearInterval(drawPoll);
        try { if (drawRtChannel) SB().removeChannel(drawRtChannel); } catch (_) {}
        drawRtChannel = null;
      };
      document.addEventListener('lotri:view', stopDrawPoll, { once: true });

      /* V103 — gad kont doub-tap sou "Ajouter le pari": si de tap rive
         prèske anmenmtan (ekran tactile), dezyèm nan ranvwaye san fè
         anyen olye de dupliquer parye a. */
      let addBetBusy = false;
      host.querySelector('#vente-ajouter').onclick = async ()=>{
        if (addBetBusy) return;
        addBetBusy = true;
        try { await addBetOnce(); }
        finally { setTimeout(()=>{ addBetBusy = false; }, 250); }
      };
      async function addBetOnce(){
        ...
        if (a < r.min) { window.Lotri.popupAlert(`Le montant minimum pour ${KEY(game)} est de ${r.min} HTG.`,'warning'); return; }
        if (r.max != null && a > r.max) { window.Lotri.popupAlert(`Le montant maximum pour ${KEY(game)} est de ${r.max} HTG.`,'warning'); return; }
        // V101 — cache SENKRON: okenn tan tann rezo pou ajoute yon pari.
        // Sèvè a (jl9_rpc_create_ticket) rete dènye ranpa reyèl la.
        const blocked = window.Lotri.security.checkBallLimit(sel.value, game, n, a);
        if (blocked) { window.Lotri.popupAlert(blocked, 'warning'); return; }
        window.Lotri.security.warmBallLimits(sel.value); // rafrechi cache an background
      ...
      /* V99 — tank yon fich ki echwe enprime rete afiche (an atant retry),
         nouvo fich pa ka fè overlap sou li. */
      let pendingReceiptOpen = false;

      host.querySelector('#fiche-imprimer').onclick = async ()=>{
        if (!lines.length) { window.Lotri.toast('Aucun pari','error'); return; }
        if (pendingReceiptOpen) {
          window.Lotri.toast('Yon fich ap tann enprime anba a — fèmen (×) oswa reyisi enprime l anvan.', 'error');
          return;
        }
        openFichePreview();
      };

      /* V99 — APÈSI fich la pandan 3s ak yon bouton [Continuer] (sote
         dekont la) nan menm plas [Imprimer] la te ye, ak yon bouton X
         (anile) sou tèt li.
         · Kontdaoun fini san koupe, OSWA [Continuer] klike → finalize
           VRÈMAN (kreye tikè + tante enprime otomatikman, san popup).
         · X klike → anile: `lines` pa touche, ajan ka modifye. */
      function openFichePreview(){
        ...
        back.innerHTML = `
          <div class="modal jl-fiche-preview-modal" role="dialog" aria-modal="true">
            ...
            <div class="fiche-wrap jl-print-stage" style="max-width:320px;margin:.5rem auto 0;position:relative">
              <div class="jl-printer-slot" aria-hidden="true"></div>
              ${window.Lotri.renderFiche(draft)}
              <div class="jl-print-flash" aria-hidden="true"></div>
            </div>
            <p class="muted jl-fiche-preview-note">La fiche s\'imprime automatiquement dans <span id="jl-fiche-cd2">5</span>s.</p>
            <button type="button" class="btn btn-primary jl-fiche-continue" style="width:100%;margin-top:.5rem">
              <i class="fa-solid fa-forward"></i> Continuer</button>
            <button type="button" class="jl-fiche-x" aria-label="Fermer l\'aperçu, annuler">
              <i class="fa-solid fa-xmark"></i> Anile</button>
          </div>`;
        document.body.appendChild(back);
        const stage = back.querySelector('.jl-print-stage');
        // V99 — efè "fich k ap soti nan yon enprimant" pandan tout apèsi 3s la.
        stage?.classList.add('jl-printing');
        window.JadStackSound && window.JadStackSound.printing();
        ...
        const finish = async () => {
          if (cancelled) return;
          cancelled = true; // anpeche 2 finish() (kontdaoun + Continuer klike anmenmtan)
          clearInterval(timerId);
          // V99 — retire "enprimant" lan ak yon ti flash pal anba fich la
          // anvan nou fèmen popup la pou vre.
          stage?.classList.remove('jl-printing');
          stage?.classList.add('jl-print-done');
        ...
        back.querySelector('.jl-fiche-continue').onclick = finish;
      ...
      /* V103 — gad re-antre nan commitTicket: menm si yon apèl doub ta
         eskape lòt gad yo (cancelled, pendingReceiptOpen), commitTicket
         pa janm kreye 2 tikè pou menm fich la. */
      let commitTicketBusy = false;
      async function commitTicket(snapshot, total){
        if (commitTicketBusy) return;
        commitTicketBusy = true;
        try { await commitTicketOnce(snapshot, total); }
        finally { commitTicketBusy = false; }
      }
      async function commitTicketOnce(snapshot, total){
        ...
        if (error) {
          /* V101 — si sèvè a reponn "Tiraj X fèmen" (ka rive si l fèmen
             pandan apèsi 5s la), pa montre erè teknik la: netwaye fich
             la nan tiraj sa a epi rafrechi lis la san blame ajan an. */
          if (/f[eè]rm/i.test(error.message || '')) {
            snapshot.forEach(l => {
              const idx = lines.findIndex(x => x.draw_id === l.draw_id);
              if (idx !== -1) lines.splice(idx, 1);
            });
            refresh();
            if (typeof refreshDrawList === 'function') refreshDrawList();
            window.Lotri.popupAlert('Ce tirage vient de fermer — il a été retiré automatiquement. Vérifiez la fiche.', 'error');
            return;
          }
          window.Lotri.popupAlert(error.message,'error'); return;
        }
        ...
        /* V102 — Nou rele Maryaj Kado a DIRÈKTEMAN isit (avan enprime),
           pou si li genyen, li ka parèt sou MENM fich la (liy "MARG" anba).
           Wrap otomatik la (apre 400ms) rete tou kòm sekou — RPC la deja
           pwoteje kont doub-bay ('already'), e MG.showAward gen yon garde
           kont doub-popup pou menm tikè a. */
        let mgAward = null;
        try {
          const mg = window.Lotri && window.Lotri.v28 && window.Lotri.v28.mg;
          if (mg && mg.claim) {
            const r = await mg.claim(data.id);
            if (r && r.awarded) mgAward = { number: r.number, amount: r.amount };
          }
        } catch(_) {}
        /* V99-fix — SÈL chemen enpresyon an se renderReceipt()->reallyPrint()
           (rele pon natif la, san popup). Ansyen apèl doub
           JadStackQ2iPOS.onTicket() retire pou evite 2 tantativ ki
           kontredi youn lòt ak yon lòt fòma. */
        renderReceipt(data, receiptLines, mgAward);
      }
      ...
      function ficheData(t, receiptLines, mgAward){
        ...
        return {
          ...
          number: t.ticket_no,
          // V102 — fòma konplè: "Date : le JJ/MM/AAAA" + "Heure : HH:MM am/pm"
          date: window.Lotri.fmtFicheDate(d), time: window.Lotri.fmtFicheTime(d),
          currency: 'HTG', total: t.total,
          mgAward: mgAward || null,
          draws: (receiptLines||[]).map(l => ({
            draw_name: l.draw_name,
            // V102 — kòd kout (BOR/LOT3/LOT4/LOT5/MAR) olye kòd nimerik la
            bets: (l.bets||[]).map(b => ({ game_code: b.game_code,
                                           game_label: window.Lotri.gameShortLabel(b.game_code),
                                           number: b.n, amount: b.a }))
          }))
        };
      }

      /* V99-fix — Etap konfimasyon anndan menm kat la (JAMEN yon popup
         navigatè/WebView ki ka bloke apk la san bouton retou) :
         1) Bouton "Imprimer" -> animasyon + son "printing" ~3s.
         2) Apre 3s, de bouton parèt: [Kontinye] (enprime pou vre) ak
            [x] (tounen sou preview la san enprime).
         3) [Kontinye] -> rele pon natif la; siksè = son + toast +
            efase fich la; echèk = son negatif + animasyon dirab ki
            rete afiche ak bouton [Réessayer].
         4) [x] -> anile, tounen sou preview senp ak bouton [Imprimer]. */
      function renderReceipt(t, receiptLines, mgAward){
        const f = ficheData(t, receiptLines, mgAward);
        pendingReceiptOpen = true;
        /* V100 — X flotan ki te chita sou tèt "Impression en cours…" a
           (kote li te ka chevoche/mal plase) retire nèt. Nan plas li, YON
           SÈL bouton "switch" entelijan anba fich la: li chanje FAS li
           (tèks + ikòn + fon) selon si yon enpresyon ap fèt oswa non — pa
           2 eleman k ap goumen pou menm plas la ankò. */
        host.querySelector('#impression-resultat').innerHTML = `
          <div class="fiche-wrap jl-receipt-wrap" style="max-width:340px;margin-top:1rem;position:relative">
            <div class="jl-print-anim" aria-hidden="true"><span class="jl-print-anim-bar"></span><span class="jl-print-anim-label">Enpresyon ap fèt…</span></div>
            <div class="jl-printer-slot" aria-hidden="true"></div>
            ${window.Lotri.renderFiche(f)}
            <div class="jl-print-flash" aria-hidden="true"></div>
            <div class="jl-print-actions" style="margin-top:.75rem;display:flex;gap:.5rem">
              <button class="btn btn-primary btn-sm jl-anim-focus" style="flex:1" id="pf">
                <i class="fa-solid fa-print"></i> Imprimer Ticket</button>
            </div>
            <div class="jl-print-confirm" style="display:none;margin-top:.5rem;gap:.5rem">
              <button type="button" class="btn btn-primary btn-sm jl-anim-focus" id="pf-continue" style="flex:1">
                <i class="fa-solid fa-check"></i> Kontinye — Imprimer</button>
            </div>
            <button type="button" class="jl-cancel-toggle jl-anim-focus" id="jl-cancel-toggle" data-state="idle">
              <span class="jl-ct-face jl-ct-idle"><i class="fa-solid fa-xmark"></i><span>Anile fich la</span></span>
              <span class="jl-ct-face jl-ct-busy"><i class="fa-solid fa-spinner fa-spin"></i><span>Enpresyon ap fèt…</span></span>
            </button>
          </div>`;

        const wrap = host.querySelector('.jl-receipt-wrap');
        const btn = host.querySelector('#pf');
        const actionsRow = host.querySelector('.jl-print-actions');
        const confirmRow = host.querySelector('.jl-print-confirm');
        const btnContinue = host.querySelector('#pf-continue');
        const cancelToggle = host.querySelector('#jl-cancel-toggle');

        // Yon sèl fonksyon santral ki jere de fas BTN switch la ansanm ak
        // eta reyèl enpresyon an — yo pa janm ka dekole youn de lòt.
        function setCancelState(printing){
          cancelToggle.dataset.state = printing ? 'busy' : 'idle';
          cancelToggle.disabled = !!printing;
          cancelToggle.setAttribute('aria-label', printing ? 'Enpresyon ap fèt, tanpri tann' : 'Anile fich la');
        }

        cancelToggle.onclick = () => {
          if (cancelToggle.dataset.state === 'busy') return; // entelijan: pa reyaji pandan enpresyon
          window.JadStackSound && window.JadStackSound.click();
          pendingReceiptOpen = false;
          host.querySelector('#impression-resultat').innerHTML = '';
        };

        function startConfirmStep(){
          // Etap 1 : animasyon + son pandan ~3s, san enprime pou vre ankò.
          actionsRow.style.display = 'none';
          wrap.classList.add('jl-printing');
          setCancelState(true);
          window.JadStackSound && window.JadStackSound.printing();
          setTimeout(() => {
            wrap.classList.remove('jl-printing');
            // V99 — retire "enprimant" lan ak yon ti animasyon + flash pal
            // anba fich la, pou montre klèman fich la fin soti.
            wrap.classList.add('jl-print-done');
            setTimeout(() => wrap.classList.remove('jl-print-done'), 900);
            confirmRow.style.display = 'flex';
            setCancelState(false);
          }, 5500);
        }

        async function reallyPrint(){
          btnContinue.disabled = true;
          btnContinue.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Impression…';
          wrap.classList.add('jl-printing');
          setCancelState(true);
          window.JadStackSound && window.JadStackSound.printing();

          if (t && t.id) {
            try { await SB().rpc('jl17_rpc_mark_printed', { _ticket: t.id }); } catch(_) {}
          }
          const r = await window.Lotri.printFiche(f);
          wrap.classList.remove('jl-printing');

          if (r && r.ok) {
            window.JadStackSound && window.JadStackSound.success();
            window.Lotri.toast('Fiche imprimée avec succès ✅', 'success');
            // V99 — kout tan pou montre "enprimant" la retire l ak flash lan
            // anvan nou efase fich la nèt. V100 — apre sa, ZERO tras fich la
            // dwe rete sou paj vant la: nou netwaye #impression-resultat
            // nèt epi refresh() (apèl pi ba, nan commitTicket) deja te vide
            // `lines`/inputs yo pou pare pou yon nouvo fich.
            wrap.classList.add('jl-print-done');
            await new Promise(res => setTimeout(res, 900));
            pendingReceiptOpen = false;
            host.querySelector('#impression-resultat').innerHTML = '';
          ...
          window.JadStackSound && window.JadStackSound.fail();
          const msg = (r && /disponib.*APK|non connect|pa konekte/i.test(r.message||''))
            ? 'Okenn enprimant konekte.'
            : (r && r.message) || 'Échec de l\'impression.';
          window.Lotri.toast(msg, 'error');
          wrap.classList.add('jl-print-error');
          confirmRow.style.display = 'none';
          actionsRow.style.display = 'flex';
          btn.disabled = false;
          btn.innerHTML = '<i class="fa-solid fa-print"></i> Réessayer l\'impression';
          btnContinue.disabled = false;
          btnContinue.innerHTML = '<i class="fa-solid fa-check"></i> Kontinye — Imprimer';
          setCancelState(false);
        }

        btn.onclick = () => { wrap.classList.remove('jl-print-error'); startConfirmStep(); };
        btnContinue.onclick = reallyPrint;
      }
    }
  });
  ...
  LotriShell.register('tickets', {
    render: async (host)=>{
        ...
        list.innerHTML = (data||[]).length
          ? `<div class="table-wrap"><table class="table"><thead><tr><th>#</th><th class="num">Total</th><th class="num">Prime</th><th>Statut</th><th>Imprimer</th><th>Date</th><th></th></tr></thead>
             <tbody>${data.map(t=>`<tr data-ticket="${t.id}"><td class="mono" data-copy-value="${esc(t.ticket_no)}">${esc(t.ticket_no)}</td>
               ...
               <td><span class="badge ${t.status==='won'?'badge-success':t.status==='cancelled'?'badge-danger':t.status==='pending_delete'?'badge-warning':''}">${t.status==='pending_delete' ? 'En attente d\'approbation' : esc(t.status)}</span></td>
               ...
               <td style="display:flex;gap:.4rem;flex-wrap:wrap">
                  ${t.status!=='pending_delete' && t.status!=='cancelled'
                    ? `<button class="btn btn-sm btn-ghost" data-reprint="${t.id}" title="${t.printed_at ? 'Réimprimer (déjà imprimée)' : 'Imprimer'}"><i class="fa-solid fa-print"></i> ${t.printed_at ? 'Réimprimer' : 'Imprimer'}</button>`
                    : ''}
                  ${t.status==='pending_delete'
                    ? '<span class="muted" style="font-size:.78rem"><i class="fa-solid fa-hourglass-half"></i> Demande envoyée</span>'
                    : t.status==='active'
                    ? (selfWindow(t)
      ...
      /* V101 — Réimpression depuis "Mes tickets": reconstruit la fiche à
       * partir des données déjà enregistrées (t.bets) même si la fiche a
       * déjà été imprimée auparavant (t.printed_at rempli). Ne crée
       * JAMAIS un nouveau ticket — seulement une nouvelle impression du
       * même ticket. */
      async function reprintTicket(id){
        const btn = host.querySelector(`[data-reprint="${id}"]`);
        if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> …'; }
        try{
          const { data: t, error } = await SB().from('jl9_tickets').select('*').eq('id', id).maybeSingle();
          if (error || !t) { window.Lotri.toast((error && error.message) || 'Fiche introuvable.', 'error'); return; }
          const bets = Array.isArray(t.bets) ? t.bets : [];
          const drawIds = [...new Set(bets.map(b => b.draw_id).filter(Boolean))];
          let drawNames = {};
          if (drawIds.length) {
            const { data: draws } = await SB().from('jl9_draws').select('id,name').in('id', drawIds);
            (draws || []).forEach(d => { drawNames[d.id] = d.name; });
          }
          const byDraw = {};
          bets.forEach(b => {
            const key = b.draw_id || 'na';
            if (!byDraw[key]) byDraw[key] = { draw_name: drawNames[b.draw_id] || t.ticket_no, bets: [] };
            byDraw[key].bets.push({ n: b.n, a: b.a, game_code: b.game_code });
          });
          const co = (window.__lotriCompany || {});
          const d = new Date(t.created_at || Date.now());
          const f = {
            company: { name: co.name || (window.Lotri.config.brand || {}).name || 'JADSTACK LOTTO', address: co.address || '', phone: co.phone || '' },
            ref: t.ticket_no, serial: t.serial || t.id || t.ticket_no, number: t.ticket_no,
            date: d.toLocaleDateString('fr-HT'), time: d.toLocaleTimeString('fr-HT'),
            currency: 'HTG', total: t.total,
            draws: Object.values(byDraw).map(l => ({
              draw_name: l.draw_name,
              bets: l.bets.map(b => ({ game_code: b.game_code, game_label: b.game_code, number: b.n, amount: b.a }))
            }))
          };
          const r = await window.Lotri.printFiche(f);
          if (r && r.ok) {
            window.JadStackSound && window.JadStackSound.success();
            window.Lotri.toast('Fiche réimprimée ✅', 'success');
            try { await SB().rpc('jl17_rpc_mark_printed', { _ticket: t.id }); } catch (_) {}
          } else {
            window.JadStackSound && window.JadStackSound.fail();
            window.Lotri.toast((r && r.message) || 'Échec de la réimpression.', 'error');
          }
        } catch (e) {
          window.Lotri.toast(e.message || 'Impossible de réimprimer.', 'error');
        } finally {
          if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-print"></i> Réimprimer'; }
        }
      }
      ...
      host.addEventListener('click', async(e)=>{
        ...
        const rep = e.target.closest('[data-reprint]');
        if (rep){ await reprintTicket(rep.dataset.reprint); return; }
        if (del){
...
(function () {
  ...
  function build() {
    ...
    /* V100 — paj koneksyon an dwe toujou parèt an FRANSE, kèlkeswa lang
       yo te chwazi anvan (localStorage 'jl:lang'): se PREMYE ekran moun
       nan wè anvan menm yo konekte, donk nou fikse l an fransè epi nou
       eskli l de sistèm tradiksyon otomatik la (data-no-i18n) pou li
       pa aksidantèlman tradwi tèks fòm nan nan yon lòt lang. */
    ...
    el.innerHTML = `
      <form class="jl27-login-card" id="jl27-login-form" autocomplete="on">
        <div class="jl27-login-brand">
          ...
          <p class="jl27-domain">jadstacklotto.com</p>
        </div>
        ...
        <label class="jl27-remember">
          <input type="checkbox" id="jl27-remember" checked>
          <span>Rester connecté sur cet appareil</span>
        </label>
        <button class="jl27-submit" type="submit" id="jl27-submit">
          <i class="fa-solid fa-right-to-bracket"></i> Se connecter
        </button>
        <!-- V103 — Yon Agent pa kapab reyinisyalize modpas li limenm ankò
             (ansyen bouton "Mot de passe oublié ?" ki te rele
             resetPasswordForEmail() retire). Sèl Konpayi a oswa
             Super-Admin ka chanje modpas yon Agent, kidonk isit se sèlman
             yon nòt enfòmatif, pa yon aksyon. -->
        <p class="jl27-link jl27-link--static">
          Mot de passe oublié ? <strong>Contactez votre compagnie ou le Super Admin.</strong>
        </p>
      </form>`;
    return el;
  }
  ...
  function show() {
    ...
    $('#jl27-login-form').onsubmit = async (e) => {
      ...
      try {
        ...
        /* V99 — "Rete konekte" : si ajan an DEKOCHE l, nou make preferans lan
           pou auth-guard.js dekonekte otomatikman sesyon an nan pwochen
           chajman app la (li pa afekte sesyon KOUNYE A a). */
        try {
          const stay = $('#jl27-remember')?.checked !== false;
          localStorage.setItem('jl_stay_signed_in', stay ? '1' : '0');
        } catch(_) {}
        if (prof.role !== 'agent') {
    ...
    render: async (host) => {
      ...
      <div class="jl27-page">
        ...
        <div class="card jl27-filters">
          ...
          <div class="jl27-frow">
            <label class="jl27-f"><span class="label">Du</span>
              <input class="input" type="date" id="jl27-date" value="${today()}" max="${today()}"></label>
            <label class="jl27-f"><span class="label">Jusqu'à</span>
              <input class="input" type="date" id="jl27-date-end" value="${today()}" max="${today()}"></label>
      ...
      async function load() {
        ...
        const dayEnd = $('#jl27-date-end').value || day;
        const drawId = $('#jl27-draw').value;
        const game = $('#jl27-game').value;
        const status = $('#jl27-status').value;
        $('#jl27-day').textContent = (dayEnd === day)
          ? new Date(day + 'T12:00:00').toLocaleDateString('fr-HT')
          : new Date(day + 'T12:00:00').toLocaleDateString('fr-HT') + ' → ' + new Date(dayEnd + 'T12:00:00').toLocaleDateString('fr-HT');
      ...
      ['#jl27-date', '#jl27-date-end', '#jl27-draw', '#jl27-game', '#jl27-status'].forEach(s => {
      ...
      /* V100-fix · rafrechi lis la lè yon fich efase/mande anile nan modal Détails */
      const onTicketChanged = () => load().catch(e => window.Lotri.toast(e.message, 'error'));
      document.addEventListener('lotri:ticket-changed', onTicketChanged);
      document.addEventListener('lotri:view', () => document.removeEventListener('lotri:ticket-changed', onTicketChanged), { once: true });
    }
  });
})();
    ...
    if(!modal){
      ...
      modal.innerHTML=`<div class="v72-settings-backdrop" data-v72-close></div><section class="v72-settings-panel" role="dialog" aria-modal="true" aria-labelledby="v72-settings-title">
        ...
        <div class="v72-settings-body">
          ...
          <h3>Son</h3>
          <label class="v72-silent-row" for="v72-silent-toggle">
            <span><i class="fa-solid fa-volume-xmark"></i> Mode silansye</span>
            <span class="v72-switch"><input type="checkbox" id="v72-silent-toggle"><span class="v72-switch-track"></span></span>
          </label>
        </div>
      </section>`;
      ...
      /* V99-fix — Mode silansye, DEZAKTIVE pa defo (son yo jwe). */
      const silentInput = modal.querySelector('#v72-silent-toggle');
      silentInput.checked = !!(window.JadStackSilent && window.JadStackSilent.isOn());
      silentInput.addEventListener('change', ()=>{
        window.JadStackSilent && window.JadStackSilent.set(silentInput.checked);
        if (!silentInput.checked) window.JadStackSound && window.JadStackSound.click();
      });
    }
...
(async function(){
  ...
  try {
    ...
    /* V99 — Ajan te dekoche "Rete konekte" — pa restore sesyon an
       otomatikman nan yon nouvo chajman app; dekonekte epi montre login. */
    if (s && localStorage.getItem('jl_stay_signed_in') === '0') {
...
(function () {
  ...
  // SÈL SOUS VERITE = BAZ DONE: yon tiraj parèt isit si e sèlman si li `active`
  // nan jl11_draw_media (kolòn ke Super-Admin kontwole nan "Tirage & logo").
  // (Ansyen lis KEEP an dur te retire — li te ka gen non tiraj ki pa egziste
  // ankò nan baz done a (drift), e li te kache tiraj reyèl ki aktif.)
  ...
  async function loadDraws(dateStr) {
    const drawDate = dateStr || today();
  ...
  function renderStrip(bar, draws) {
  ...
  /* V101 — cache «stale-while-revalidate»: montre imedyatman dènye
     rezilta konnen yo (localStorage) pandan nou rafrechi an background,
     olye montre yon spinner vid chak fwa paj la chanje. */
  async function paintHeader() {
    ...
    try {
      const key = 'v11-draws-' + today();
      const { data: cached } = window.Lotri.cache.swr(key, loadDraws, fresh => {
        const liveBar = document.getElementById('v11-draws');
        if (liveBar) renderStrip(liveBar, fresh);
      }, 5 * 60 * 1000);
      const draws = cached || await loadDraws();
      renderStrip(bar, draws);
    } finally {
  ...
  /* ---------- Sou-paj «rezilta jadya» — ak filtraj pa dat (istorik) ---------- */
    ...
    render: async (host) => {
      ...
      host.innerHTML = `
        ...
        <div class="page-hd"><h2>Résultats des tirages</h2>
          <p class="muted" id="v77-res-sub">Chargement…</p></div>
        <div class="card v77-results-filter" style="display:flex;align-items:center;gap:.6rem;flex-wrap:wrap;margin-bottom:.8rem">
          <label class="label" style="margin:0" for="v77-res-date"><i class="fa-solid fa-calendar-days"></i> Dat</label>
          <input class="input" type="date" id="v77-res-date" style="max-width:180px" value="${esc(state.date)}" max="${esc(today())}">
          <button type="button" class="btn btn-ghost btn-sm" id="v77-res-today">Jodi a</button>
          <button type="button" class="btn btn-ghost btn-sm" id="v77-res-prev"><i class="fa-solid fa-chevron-left"></i> Vèy</button>
          <button type="button" class="btn btn-ghost btn-sm" id="v77-res-next">Demen <i class="fa-solid fa-chevron-right"></i></button>
        </div>
        <div class="card v77-results-card"><div class="table-wrap" id="v77-res-table"><div class="spinner"></div></div></div>`;

      const dateInput = host.querySelector('#v77-res-date');
      ...
      async function paint() {
        const wrap = host.querySelector('#v77-res-table');
        const sub = host.querySelector('#v77-res-sub');
        wrap.innerHTML = '<div class="spinner"></div>';
        const isToday = state.date === today();
        const draws = await loadDraws(state.date);
        const pending = draws.filter(d => !d.result);
        sub.textContent = isToday
          ? (pending.length ? pending.length + ' tirages n\'ont pas encore de résultat pour aujourd\'hui.' : 'Tous les tirages ont un résultat pour aujourd\'hui.')
          : `Résultats du ${displayDate(state.date)}${pending.length ? ' — ' + pending.length + ' tirage(s) sans résultat ce jour-là' : ''}`;
        ...
        wrap.innerHTML = `<table class="table">
          ...
          <tbody>${draws.map(d => {
            ...
            return `<tr data-v11-title="${esc(d.display_name)}" data-v11-row='${esc(JSON.stringify({
              Tirage: d.display_name, Dat: state.date, Fèmti: d.close_time || '—',
              ...
              <td>${r.scope === 'company' ? '<span class="badge">Autre bolèt</span>'
                ...
                ${isToday && r.updated_at && (Date.now() - new Date(r.updated_at).getTime()) < 3 * 3600 * 1000
                  ? '<span class="badge badge-danger" style="margin-left:.3rem">Nouvo</span>' : ''}</td>
              <td>${(isToday && canEdit(p.role))
                  ? `<button class="btn btn-sm" data-open="${d.id}"><i class="fa-solid fa-pen"></i> Saisir le résultat</button>`
                  : (r.lot1 ? `<button class="btn btn-sm" data-view="${d.id}"><i class="fa-solid fa-eye"></i> Voir</button>` : '<span class="muted">—</span>')}</td>
            </tr>`;
          }).join('') || `<tr><td colspan="7" class="muted" style="text-align:center;padding:1.2rem">Okenn tiraj pou dat sa a.</td></tr>`}</tbody></table>`;
        ...
        wrap.querySelectorAll('[data-view]').forEach(b => b.onclick = e => {
          e.stopPropagation();
          const d = draws.find(x => x.id === b.dataset.view);
          if (!d) return;
          const pop2 = v11.popup('Résultats des tirages', `
            <div style="display:flex;gap:.9rem;align-items:center">${ball(d)}
              <div><strong>${esc(d.display_name)}</strong>
              <div class="muted" style="font-size:.75rem">Fèmti: ${esc(d.close_time || '—')} · ${displayDate(state.date)}</div></div></div>
            <div style="margin-top:1rem;text-align:center;font-size:1.4rem">${lots(d.result)}</div>`,
            { subtitle: d.display_name, footer: `<button class="btn btn-ghost" id="goto">Fèmen</button>` }
          );
          pop2.el.querySelector('#goto').onclick = pop2.close;
        });
      }

      dateInput.onchange = () => { state.date = dateInput.value || today(); paint(); };
      host.querySelector('#v77-res-today').onclick = () => { state.date = today(); dateInput.value = state.date; paint(); };
      host.querySelector('#v77-res-prev').onclick = () => { state.date = shiftDate(state.date, -1); dateInput.value = state.date; paint(); };
      host.querySelector('#v77-res-next').onclick = () => {
        const nxt = shiftDate(state.date, 1);
        if (nxt > today()) return;
        state.date = nxt; dateInput.value = state.date; paint();
      };
    ...
    render: async (host) => {
      ...
      host.innerHTML = `
        ...
        <div class="card"><div class="card-hd"><h3>Liste des tirages</h3>
          <button class="btn btn-primary" id="new"><i class="fa-solid fa-plus"></i> Nouveau tirage</button></div>
...
(function () {
  ...
  /* V104 — "Rejouer" yon ansyen fich (bouton nan Détails du ticket).
     Nou SÈLMAN mete pari yo nan yon file datant (window.__lotriReplayQueue)
     epi n ale sou paj POS la; se paj POS la ki deside si tiraj la (menm
     non) toujou louvri kounye a anvan l pre-ranpli fich la. Okenn tikè
     PA kreye/enprime otomatikman isit la — ajan an toujou oblije tcheke
     epi peze "Imprimer" li menm, kidonk tout validasyon sèvè a
     (jl9_rpc_create_ticket) toujou aplike jan yo te ye a. */
  window.Lotri.replayTicket = function (draw, bets) {
    window.__lotriReplayQueue = {
      drawName: (draw && draw.name) || '',
      bets: (Array.isArray(bets) ? bets : []).map(b => ({
        n: b.n ?? b.number ?? '',
        a: Number(b.a ?? b.amount ?? 0),
        game_code: b.game_code || b.game || ''
      })).filter(b => b.n && b.a > 0)
    };
    if (window.LotriShell) window.LotriShell.go('pos');
  };
  ...
  v13.ticketDetail = async function (id, opts) {
    try {
      ...
      /* V104 — menm règ egzakteman ak "Mes tickets"/showTicketDetail:
         10 min apre enprime (oswa apre kreye si poko enprime) ajan an ka
         siprime li menm; apre sa se yon demande ki voye bay administrasyon. */
      const MIN10 = 10 * 60 * 1000;
      const _selfWindow = (() => {
        const base = _t.printed_at ? new Date(_t.printed_at).getTime()
                   : (_t.created_at ? new Date(_t.created_at).getTime() : null);
        return base ? (Date.now() - base) <= MIN10 : true;
      })();
      const _canDelete = _t.status === 'active';
      const _canReplay = Array.isArray(_t.bets) && _t.bets.length > 0;
      const m = v13.modal(`<h3>Détails du ticket</h3>${v13.ticketHtml(d, opts)}
        ...
        <div class="row" style="justify-content:flex-end;margin-top:.8rem;gap:.4rem">
          ...
          ${_canReplay ? `<button class="btn btn-ghost" id="jl13-rejwe">
          ...
          ${_canDelete ? (_selfWindow
              ? `<button class="btn btn-danger" id="jl13-del"><i class="fa-solid fa-trash"></i> Supprimer</button>`
              : `<button class="btn btn-ghost" id="jl13-req"><i class="fa-solid fa-paper-plane"></i> Demander l'annulation</button>`)
            : ''}
      ...
      if (_canReplay) {
        m.el.querySelector('#jl13-rejwe').onclick = () => {
          m.close();
          window.Lotri.replayTicket(d.draw, _t.bets);
        };
      }
      ...
      if (_canDelete && _selfWindow) {
        const delBtn = m.el.querySelector('#jl13-del');
        if (delBtn) delBtn.onclick = async () => {
          const ok = await window.Lotri.ui.confirm('Supprimer ce ticket ?', 'Il sera déplacé vers la corbeille.');
          if (!ok) return;
          const { error: delErr } = await SB().rpc('jl9_rpc_delete_ticket', { _ticket: _t.id });
          if (delErr) { v13.toast(delErr.message, 'error'); return; }
          v13.toast('Ticket supprimé', 'success');
          m.close();
          document.dispatchEvent(new CustomEvent('lotri:ticket-changed', { detail: { id: _t.id, action: 'delete' } }));
        };
      } else if (_canDelete && !_selfWindow) {
        const reqBtn = m.el.querySelector('#jl13-req');
        if (reqBtn) reqBtn.onclick = async () => {
          const reason = prompt('Pourquoi voulez-vous annuler ce ticket ? (L\'administration en prendra connaissance)');
          if (reason === null) return;
          const { error: reqErr } = await SB().rpc('jl17_rpc_request_delete_ticket', { _ticket: _t.id, _reason: reason || null });
          if (reqErr) { v13.toast(reqErr.message, 'error'); return; }
          v13.toast('Demande envoyée — en attente d\'approbation.', 'success');
          m.close();
          document.dispatchEvent(new CustomEvent('lotri:ticket-changed', { detail: { id: _t.id, action: 'request' } }));
        };
      }
...
(function () {
  ...
  function betsHtml(bets) {
    ...
    return list.map(b => {
      ...
      /* V100-fix · fòma reyèl la itilize `a` pou montan (v. jl9_tickets.bets) */
      const amt = b.amount ?? b.mise ?? b.price ?? b.montan ?? b.a ?? 0;
      const gm = b.game ?? b.game_code ?? '';
      return `<span class="chip" style="margin:.15rem .2rem 0 0">${esc(num)}${gm ? ' · ' + esc(gm) : ''} = ${esc(money(amt))}</span>`;
    }).join('');
  }

  /* V100-fix · pran bets yo kèlkeswa fòma (Array oswa string JSON) */
  const betsOf = (v) => {
    if (Array.isArray(v)) return v;
  ...
  L.showTicketDetail = async function (idOrNo) {
    ...
    /* V100-fix · vrè bets fich la (jl9_tickets.bets — chak parye gen pwòp draw_id) */
    const allBets = betsOf(t.bets);

    /* V23 · C2-b — chaje TOUT tiraj ki sou fich la (jl9_ticket_draws).
       V100-fix · tab sa a se sèlman yon jonksyon (id, ticket_id, draw_id,
       created_at) — li PA gen kolòn bets/subtotal/status/prize_amount/win_detail,
       se sa k te fè "Boule & Prix" parèt vid tout tan. Nou sèvi ak li sèlman
       pou non ak lis tiraj yo, e nou pran parye reyèl yo nan t.bets. */
    ...
    const drawSectionsHtml = drawLines.length
      ? drawLines.map(d => {
          const drawBets = allBets.filter(b => String((b && b.draw_id) || '') === String(d.draw_id));
          /* Si okenn parye pa gen draw_id (ansyen fich), epi gen yon sèl tiraj, montre tout bets yo */
          const shown = drawBets.length ? drawBets : (drawLines.length === 1 ? allBets : []);
          const subtotal = shown.reduce((s, b) => s + Number((b && (b.a ?? b.amount)) || 0), 0);
          return `
        <div style="margin:.5rem 0;padding:.5rem;border:1px solid var(--border);border-radius:.4rem">
          ...
          <div>${betsHtml(shown)}</div>
          <div class="row" style="gap:1rem;margin-top:.35rem;font-size:.85rem">
            <div><span class="muted">Sou-total :</span> <strong>${esc(money(subtotal))}</strong></div>
          </div>
        </div>`;
        }).join('')
      : `<div style="margin:.35rem 0 .2rem"><span class="muted">Boule &amp; Prix :</span></div><div>${betsHtml(allBets)}</div>`;
    ...
    /* V100-fix · bouton Supprimer/Demander l'annulation, kote ou te make li a
       (menm règ ak paj "Mes tickets" : 10 min apre enprime, epi toujou avan
       tiraj la fèmen ; apre sa se yon demande ki voye bay Super Admin). */
    const MIN10 = 10 * 60 * 1000;
    const stillInSelfWindow = (() => {
      const base = t.printed_at ? new Date(t.printed_at).getTime() : null;
      return base ? (Date.now() - base) <= MIN10 : true;
    })();
    const actionHtml = t.status === 'active'
      ? (stillInSelfWindow
          ? `<button class="btn btn-sm btn-danger" data-td-del="${esc(t.id)}"><i class="fa-solid fa-trash"></i> Supprimer</button>`
          : `<button class="btn btn-sm btn-ghost" data-td-req="${esc(t.id)}"><i class="fa-solid fa-paper-plane"></i> Demander l'annulation</button>`)
      : (t.status === 'pending_delete'
          ? '<span class="muted" style="font-size:.78rem"><i class="fa-solid fa-hourglass-half"></i> Demande envoyée</span>'
          : '');
    ...
    m.innerHTML = `<div class="modal" style="max-width:560px">
      ...
      <div class="row" style="justify-content:space-between;align-items:center;margin-top:1rem;gap:.5rem;flex-wrap:wrap">
        <div>${actionHtml}</div>
    ...
    const delBtn = m.querySelector('[data-td-del]');
    if (delBtn) delBtn.onclick = async () => {
      const ok = await window.Lotri.ui.confirm('Supprimer ce ticket ?', 'Il sera déplacé vers la corbeille.');
      if (!ok) return;
      const { error: delErr } = await SB().rpc('jl9_rpc_delete_ticket', { _ticket: t.id });
      if (delErr) { window.Lotri.toast(delErr.message, 'error'); return; }
      window.Lotri.toast('Ticket supprimé', 'success');
      close();
      document.querySelectorAll(`[data-ticket="${t.id}"],[data-ticket-no="${t.ticket_no}"]`).forEach(row => row.remove());
      document.dispatchEvent(new CustomEvent('lotri:ticket-changed', { detail: { id: t.id, action: 'delete' } }));
    };

    const reqBtn = m.querySelector('[data-td-req]');
    if (reqBtn) reqBtn.onclick = async () => {
      const reason = prompt('Pourquoi voulez-vous annuler ce ticket ? (L\'administration en prendra connaissance)');
      if (reason === null) return;
      const { error: reqErr } = await SB().rpc('jl17_rpc_request_delete_ticket',
        { _ticket: t.id, _reason: reason || null });
      if (reqErr) { window.Lotri.toast(reqErr.message, 'error'); return; }
      window.Lotri.toast('Demande envoyée — en attente d\'approbation.', 'success');
      close();
      document.dispatchEvent(new CustomEvent('lotri:ticket-changed', { detail: { id: t.id, action: 'request' } }));
    };
  };
...
(function () {
   ...
   *
   * V102 — MG.claim kounye a rele DIRÈKTEMAN pa POS la (pou nimewo kado
   * a ka parèt sou fich la, liy "MARG"), ANSANM ak wrap otomatik la
   * (400ms apre, kòm sekou pou lòt chemen kreyasyon tikè). Pou de apèl
   * sa yo pa janm montre popup "ou resevwa kado a" DE FWA pou menm
   * tikè a, nou gade yon ti Set lokal (ticket_id deja montre).
   * ------------------------------------------------------------- */
  const _mgShown = new Set();
  MG.claim = async function (ticketId) {
    ...
    try {
      ...
      if (data && data.awarded) {
        if (!_mgShown.has(ticketId)) { _mgShown.add(ticketId); MG.showAward(data); }
        return data;
      }
      ...
      if (data && data.reason === 'already') {
        ...
        if (r && r.data && r.data.awarded) {
          if (!_mgShown.has(ticketId)) { _mgShown.add(ticketId); MG.showAward(r.data); }
          return r.data;
        }
      }
      return data || null;
...
(function () {
  ...
  L.V33_PHRASES = [
    ...
    ['Anile fich la', 'Annuler la fiche', 'Cancel the slip'],
    ['Enpresyon ap fèt…', 'Impression en cours…', 'Printing in progress…'],
    ['Enpresyon ap fèt, tanpri tann', "Impression en cours, veuillez patienter", 'Printing in progress, please wait'],
...
(function () {
  ...
  /* ---------- 4) Ripple ---------- */
  const RIPPLE_SEL = '.btn, .side-link, .jl-sheet-item, .jl-bottom-nav button, ' +
    'button:not(.btn), [role="button"], .v11-item, .v86-conv-row, .v86-aud-item, ' +
    '.trash-row, .game-tab, .chip-filter, .sec-tab';
...
(function () {
  ...
  const LOCAL = {
    wide: 'assets/img/jadstacklotto_logo.png',
    /* V98 — vèsyon "JadStack LOTTO" ak tèks BLAN, pou tèks la toujou lizib
       lè `.logo-wide` chita sou --surface fonse (sidebar/topbar) an dark
       theme. Sèlman aplike sou logo LOKAL (defo); yon logo koutwaye admin
       telechaje pa ka rekolorye otomatikman. */
    wideDark: 'assets/img/jadstacklotto_logo-dark.png',
    mark: 'assets/img/logo.png',
  };
  ...
  V.paintLogos = function (root) {
    ...
    scope.querySelectorAll('[data-brand]').forEach((host) => {
      ...
      const local = (kind === 'wide' && isDark) ? LOCAL.wideDark : LOCAL[kind];
  ...
  /* V98 — lè moun nan chanje ant tèm klè/fonse, repentire logo "wide" la
     pou l switch ant vèsyon tèks nòmal la ak vèsyon tèks blan an. */
  document.addEventListener('theme-changed', () => V.paintLogos(document));
...
(function () {
  ...
  document.addEventListener('pointerdown', function (e) {
    var t = e.target.closest && e.target.closest('.btn, .jl-bottom-nav button, .side-link, .jl-sheet-item');
    if (t) { haptic(8); if (window.JadStackSound && !t.disabled) window.JadStackSound.click(); }
  }, { passive: true });
...
/* =======================================================================
 * JADSTACK LOTTO V103 — Popup pou mesaj enpòtan (olye toast k disparèt).
 * Additif pi: pa touche `window.Lotri.toast` (toujou la pou mesaj kout,
 * pa enpòtan). Popup sa a rete afiche jiskaske ajan an fèmen l li menm
 * (bouton X oswa "Fèmen"), pou li pa janm rate yon avètisman sou lajan
 * (limit, montant min/max, tiraj ki fèmen, elatriye).
 * Zewo backend, zewo SQL — CSS li mete an liy pou fonksyone san
 * depann sou yon fichye CSS separe.
 * ===================================================================== */
(function () {
  'use strict';
  if (window.Lotri && window.Lotri.popupAlert) return; // pa doub-defini

  function ensureStyle() {
    if (document.getElementById('jl103-popup-style')) return;
    var css = '.jl103-backdrop{position:fixed;inset:0;background:rgba(10,16,28,.55);'
      + 'z-index:99999;display:flex;align-items:center;justify-content:center;padding:1rem}'
      + '.jl103-box{background:#fff;border-radius:14px;max-width:360px;width:100%;'
      + 'box-shadow:0 18px 48px rgba(0,0,0,.28);padding:1.1rem 1.1rem .9rem;position:relative;'
      + 'animation:jl103-in .16s ease-out}'
      + '@keyframes jl103-in{from{opacity:0;transform:translateY(6px) scale(.98)}to{opacity:1;transform:none}}'
      + '.jl103-box[data-kind="error"]{border-top:4px solid #dc2626}'
      + '.jl103-box[data-kind="warning"]{border-top:4px solid #d97706}'
      + '.jl103-box[data-kind="success"]{border-top:4px solid #16a34a}'
      + '.jl103-box[data-kind="info"]{border-top:4px solid #2563eb}'
      + '.jl103-hd{display:flex;align-items:flex-start;gap:.6rem}'
      + '.jl103-ic{font-size:1.3rem;line-height:1;margin-top:.1rem}'
      + '.jl103-box[data-kind="error"] .jl103-ic{color:#dc2626}'
      + '.jl103-box[data-kind="warning"] .jl103-ic{color:#d97706}'
      + '.jl103-box[data-kind="success"] .jl103-ic{color:#16a34a}'
      + '.jl103-box[data-kind="info"] .jl103-ic{color:#2563eb}'
      + '.jl103-msg{flex:1;font-size:.96rem;line-height:1.4;color:#1a2233}'
      + '.jl103-x{position:absolute;top:.5rem;right:.5rem;width:30px;height:30px;border:0;'
      + 'background:transparent;color:#6b7280;font-size:1rem;border-radius:8px;cursor:pointer}'
      + '.jl103-x:hover{background:#f1f2f5;color:#1a2233}'
      + '.jl103-btn{margin-top:.9rem;width:100%;border:0;border-radius:10px;padding:.65rem;'
      + 'font-weight:600;font-size:.94rem;cursor:pointer;background:#004E8C;color:#fff}'
      + '.jl103-btn:active{transform:translateY(1px)}';
    var st = document.createElement('style');
    st.id = 'jl103-popup-style';
    st.textContent = css;
    document.head.appendChild(st);
  }

  var ICONS = { error: 'fa-circle-exclamation', warning: 'fa-triangle-exclamation',
                success: 'fa-circle-check', info: 'fa-circle-info' };

  function popupAlert(msg, kind, opts) {
    if (msg === undefined || msg === null || msg === '') return function(){};
    ensureStyle();
    kind = kind || 'info'; opts = opts || {};
    var back = document.createElement('div');
    back.className = 'jl103-backdrop';
    back.innerHTML =
      '<div class="jl103-box" data-kind="' + kind + '" role="alertdialog" aria-modal="true">' +
        '<button type="button" class="jl103-x" aria-label="Fèmen"><i class="fa-solid fa-xmark"></i></button>' +
        '<div class="jl103-hd">' +
          '<span class="jl103-ic"><i class="fa-solid ' + (ICONS[kind] || ICONS.info) + '"></i></span>' +
          '<span class="jl103-msg"></span>' +
        '</div>' +
        '<button type="button" class="jl103-btn">' + (opts.closeLabel || 'Fèmen') + '</button>' +
      '</div>';
    back.querySelector('.jl103-msg').textContent = String(msg);
    document.body.appendChild(back);
    if (window.JadStackSound && kind === 'error' && window.JadStackSound.error) {
      try { window.JadStackSound.error(); } catch (_) {}
    }
    function close() { back.remove(); document.removeEventListener('keydown', onKey); }
    function onKey(e) { if (e.key === 'Escape') close(); }
    document.addEventListener('keydown', onKey);
    back.querySelector('.jl103-x').addEventListener('click', close);
    back.querySelector('.jl103-btn').addEventListener('click', close);
    back.addEventListener('click', function (e) { if (e.target === back) close(); });
    return close;
  }

  window.Lotri = window.Lotri || {};
  window.Lotri.popupAlert = popupAlert;
})();
